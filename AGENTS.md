...
- **Detection Worker (`public/detection-worker.js`):** OpenCV contour detection runs in a dedicated Web Worker. The main thread transfers downscaled (~360px) `ImageData`; the Worker selects qualifying external contours with strict guard rails (15-85% area, 1.12-1.88 aspect ratio, 65-115° interior angles, strict convexity, Canny 65/185, 3px boundary rejection), derives corners from its convex hull, and replies with four normalized points. Detection is throttled to ~12 fps (every 80ms) via `isDetecting` lock + `lastDetectTime` timestamp.
